...
const Navbar = ({ scrollTo }) => {
  ...
  return (
    <>
      <nav className='md:fixed sm:top-0 sm:z-10 w-full py-4 px-5  flex items-center justify-between lg:flex-row mx-auto shadow-lg bg-white'>

        <div className='h-10 w-10'>
          <span className='text-gray-500 font-mono tracking-tight'>khaig.hsu.thwe.dev@gmail.com</span>
        ...
        <div className='space-x-4 '>
          ...
          <ul className='hidden md:flex font-mono'>
            {nav_items.map((item, index) => (
              <li
                key={index}
                className={`mr-6 relative cursor-pointer  text-sm md:text-lg block after:block after:content-[''] after:absolute after:h-[3px] after:bg-[#6c63ff] after:w-full after:scale-x-0 after:hover:scale-x-100 after:transition after:duration-300 after:origin-center ${activeSection === item.link ? 'after:bg-[#6c63ff]' : ''} ${activeSection === item.link ? 'text-[#6c63ff]' : 'text-gray-500'} ${activeSection === item.link ? 'font-bold' : ''}`}
